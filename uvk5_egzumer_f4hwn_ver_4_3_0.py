...
#3.5.1: 
#       Add the modification about the duplex reporter in the issue of the github
#       update link pdf for the 3.5
 
#3.5.2: 
#       modify the upload function for the user about all the f4hwn setting
...
# add menu firmware with version and option display if version 3.0 and up
        ...
        if self.FIRMWARE_VERSION == "":
            ...
            if self.FIRMWARE_VERSION >= Compair1 :
                if _mem.BUILD_OPTIONS.ENABLE_FEAT_F4HWN_GAME and not _mem.BUILD_OPTIONS.ENABLE_BANDSCOPE :   
                    ValFirm = ValFirm + " Edition Game"
                ...
                elif _mem.BUILD_OPTIONS.ENABLE_FMRADIO and _mem.BUILD_OPTIONS.ENABLE_BANDSCOPE and _mem.BUILD_OPTIONS.ENABLE_FEAT_F4HWN_GAME :         
...
#       7 youtube channel YOUTUBE_INFO_4S7JL_SRI_LANKA
        val = RadioSettingValueBoolean(False)
        def validate_Go_Web_Page9(value):
            if value :
                msg = "Open Youtube web page from 4S7JL_SRI_LANKA \n" \
                      + YOUTUBE_INFO_4S7JL_SRI_LANKA
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
                    webbrowser.open(YOUTUBE_INFO_4S7JL_SRI_LANKA)
                value = False
                    
            return value

        val.set_validate_callback(validate_Go_Web_Page9)
        rs = RadioSetting("CHAINE_YOUTUBE9","Youtube from 4S7JL_SRI_LANKA   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)
